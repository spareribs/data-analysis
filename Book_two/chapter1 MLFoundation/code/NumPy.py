...
# 输出结果
print(u'[Info]: 生成一个4*4的随机数组randArray 类型为 {0}\n{1}'.format(type(randArray), randArray))
print(u'[Info]: 转化成矩阵randMat 类型为 {0}\n{1}'.format(type(randMat), randMat))
print(u'[Info]: 矩阵求逆invRandMat 类型为 {0}\n{1}'.format(type(invRandMat), invRandMat))
print(u'[Info]: 矩阵转置TraRandMat 类型为 {0}\n{1}'.format(type(TraRandMat), TraRandMat))
print(u'[Info]: 矩阵基于的数组ArrRandMat 类型为 {0}\n{1}'.format(type(ArrRandMat), ArrRandMat))
# 矩阵和逆矩阵 进行求积 (单位矩阵，对角线都为1嘛，理论上4*4的矩阵其他的都为0)
myEye = randMat * invRandMat
# 误差
print(u'[Info]: 误差\n{0}'.format(myEye - eye(4)))
'''
