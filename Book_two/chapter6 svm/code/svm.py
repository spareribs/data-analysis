...
def loadDataSet(fileName):
    ...
    :param fileName: 文件名
    :return: dataMat 特征矩阵;labelMat 类标签
    """
    dataMat = []
    labelMat = []
    fr = open(fileName)
...
if __name__ == "__main__":
    # 获取特征和目标变量
    dataArr, labelArr = loadDataSet(getdatapath("testSet.txt"))
    print(u"[Info]:{0}\n{1}".format(dataArr, labelArr))
    # selectJrand(i, m)
    print(u"[Info]:".format())
    # clipAlpha(aj, H, L)
    print(u"[Info]:".format())
    ...
    print(u"[Info]:{0}\n{1}".format(b, alphas[alphas > 0]))

    # 获取支持向量的数据点
    for i in range(100):
        if alphas[i] > 0:
            print(u"[Info]:{0}\n{1}".format(dataArr[i], labelArr[i]))
